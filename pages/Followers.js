  ...
  useEffect(() => {
    try {
      ...
      axios
        ...
        .then((response) => {
          response.data.donnee.publications.map((data,index)=>{
            axios
            .get(
              `http://185.98.139.246:9090/ogatemanagement-api/client/recherchetlisteutilisateursinteressesparpublication/${data.id}`,
              config
            )
